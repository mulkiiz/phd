...
function Selection($iterasi)
{
	...
	while($rr = mysql_fetch_array($result)){
		...
		// echo "ada kromosom \n";
			for ($i=1; $i<=2 ; $i++) { 
				$r = r_random(4);
				$kro = getfreecol('partikel', 'tb_fitness', 'iterasi='.$prev.' and csn >'.$r.' LIMIT 1');
				if($i==1){
...
function CrossOver($iterasi,$partikel)
{
	echo "FASE: CrossOver\n";
	$prev = $iterasi - 1;
	...
	$query_bapak = "INSERT INTO tb_matriks_t(iterasi,partikel,iddata,stat) 
			  SELECT ".$iterasi.",".$partikel.",iddata,stat FROM tb_matriks where iterasi = ".$prev." and partikel=".$bapak." LIMIT 3 OFFSET 0 ";
	echo $query_bapak."\n";
	$result_bapak = mysql_query($query_bapak) or die (mysql_error());

	$query_ibu = "INSERT INTO tb_matriks_t(iterasi,partikel,iddata,stat) 
			  SELECT ".$iterasi.",".$partikel.",iddata,stat FROM tb_matriks where iterasi = ".$prev." and partikel=".$ibu." LIMIT 17 OFFSET 3 ";
	echo $query_ibu."\n";
...
function Mutasi($iterasi,$partikel,$iddata)
{
	// echo "FASE: Mutasi STARTED\n";

	$pmutasi=0.25; $r = r_random(4);
