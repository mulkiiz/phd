...
function inKnap($ith,$par){
	...
	for ($idrute=1; $idrute<=20 ; $idrute++) {
		...
		{
			// echo "Prob: in\n";
			$city = getfreecol('min(awal)','tb_matriks',' awal not in (select rute from tb_rute WHERE iterasi='.$ith.' and partikel='.$par.') ORDER BY RAND(), vistotal DESC LIMIT 2');
	...
	updatedata('tb_fxrute',' fxtotal='.$fitness.' WHERE iterasi='.$ith.' and partikel='.$par.' ');

	updateVistho($ith,$par,$maxKnap);
?>