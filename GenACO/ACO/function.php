...
function setMaxFit($iterasi)
{
	...
	$query = "select iterasi,partikel,fitness from tb_fitness where iterasi = ".$iterasi." ORDER BY fitness DESC limit 1";
	// echo $query."\n";
...
function setMinFit($iterasi)
{
	...
	$query = "select iterasi,partikel,fitness from tb_fitness where iterasi = ".$iterasi." ORDER BY fitness ASC limit 1";
	// echo $query."\n";
...
function setOutFit($iterasi)
{
	...
	$query = "select iterasi,partikel from tb_fitness where iterasi = ".$iterasi." and fxberat >1200";
	// echo $query."\n";
...
function setKumulatif($iterasi,$kota)
{
	...
	if($kota==1) {
		$query = "select vistotal from tb_matriks where awal=".$iterasi." and akhir=".$kota." ";
		$result = mysql_query($query) or die (mysql_error());
		$kum = mysql_fetch_array($result);

		updatedata('tb_matriks'," kum=".$kum[0]." where awal=".$iterasi." and akhir=".$kota." ");
	}else{
		$query = "select vistotal from tb_matriks where awal=".$iterasi." and akhir=".$kota." ";
		$result = mysql_query($query) or die (mysql_error());
		...
		$prev = $kota - 1;
		$query = "select kum from tb_matriks where awal=".$iterasi." and akhir=".$prev." ";
		$result = mysql_query($query) or die (mysql_error());
		$kum_b = mysql_fetch_array($result);
		
		$kum_fix = $kum_a[0] + $kum_b[0];
		updatedata('tb_matriks'," kum=".$kum_fix." where awal=".$iterasi." and akhir=".$kota." ");
	}
	//mysql_close();
}
...
{
	echo "FASE: setGbest\n";
	$query = "select * from tb_fxrute where fxbobot <=1200 and iterasi=".$ith." ORDER BY fxbobot DESC limit 1";
	$result = mysql_query($query) or die (mysql_error());

	$numrows = mysql_num_rows($result);
	if($numrows == 0){
		$query = "select * from tb_fxrute where iterasi=".$ith." ORDER BY fxbobot DESC, inknap DESC limit 1";
	}else{
		$query = "select * from tb_fxrute where fxbobot <=1200 and iterasi=".$ith." ORDER BY fxbobot DESC limit 1";
	}
	$result = mysql_query($query) or die (mysql_error());
	...
	updatedata('tb_fxrute',' ispbest="ya" where id='.$row[0].' ');
